...
from framework.helpers import within_deadband, get_camera_size, ConsistencyCheck
...
def update_camera(results):
    camera_dims = get_camera_size(results)
    ...
    def on_first_run(self):
        ...
        if (Torpedoes.target.name == CutoutName.top_left or
            ...
            align_for_swipe = Concurrent(
                ...
                Sequential(
                    Timed(MoveY(0.3, deadband=0.03), 5.0),
                    Timed(MoveX(0.26), 5.0)
                )
            )
            self.remove_cover = Sequential(
                align_for_swipe, 
                Timed(MoveY(-0.6, deadband=0.05), 5.0), 
                Timed(MoveX(-0.2, deadband=0.05), 2.0),
                Timed(Concurrent(
            ...
            align_for_swipe = Concurrent(
                Timed(RelativeToInitialDepth(-0.10330, error=0.005), 5.0),
                Sequential(
                    Timed(MoveY(-0.1, deadband=0.03), 2.0),
                    Timed(MoveX(0.26), 2.0)
                )
            )
            self.remove_cover = Sequential(
                align_for_swipe, 
...
class FireTorpedo(Task):
    def on_first_run(self):
        ...
        self.torpedo_bottom_actuators = ["torpedo_bottom_1", "torpedo_bottom_2"]

        if USE_ACTUATORS:
            if Torpedoes.num_torpedoes == 1:
                self.task = Sequential(RelativeToInitialDepth(0.05, error=0.01), FireActuators(self.torpedo_bottom_actuators, 0.1))
            else:
        ...
        # Prevent drift
        shm.navigtion_desires.positional_controls.set(True)

        self.logi("Started FireTorpedo")

    def on_run(self):
        if self.task.finished:
            shm.navigtion_desires.positional_controls.set(False)
            self.finish()
        else:
            self.task()
...
class Torpedoes(Task):
    def __init__(self, cover_present="cover", *args, **kwargs):
        ...
        # TODO @JohnM: Change this when we've made progress!
