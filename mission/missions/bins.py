...
from mission.framework.helpers import get_camera, get_camera_center, ConsistencyCheck, get_sub_position
...
class BinsTask(Task):
    ...
    def on_first_run(self, *args, **kwargs):
        ...
        self.tasks = Sequential(Depth(BINS_SEARCH_DEPTH),
                                SearchBinsTask(), self.identify_bins,
...
class IdentifyBins(Task):
    ...
    def on_first_run(self, run_type, heading=None, uncovered_bin_vector=None):
        ...
        cam = get_camera(shm.bin_cover.get())
    ...
    def on_run(self, run_type, heading=None, uncovered_bin_vector=None):
        ...
        target = get_camera_center(cover_g)
...
class UncoverBin(Task):
    ...
    def on_first_run(self, slide_direction="right", *args, **kwargs):
        ...
        self.pickup = MasterConcurrent(Sequential(Timer(3.0), Roll(-30 * factor, error=90)), VelocityY(1.0 * factor))
        ...
        self.return_to_bin = Sequential(Timed(VelocityY(-1.0 * factor), 2.5), VelocityY(0.0), MoveX(-BINS_CAM_TO_ARM_OFFSET))
