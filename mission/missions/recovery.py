    ...
    def on_first_run(self, vision, *args, **kwargs):
        ...
        if stacks_on_tower > 0:
            center_tower = SequentialSuccess(
                Log('Centering tower'),
                DownwardTargetObjects(vision, lambda: vision.stacks, precision=stacks_on_tower == 4),
                Zero(),
            )
        else:
...
MoveGrabberToCamera = lambda: MoveXY((-0.04064, 0.1), deadband=0.008)
...
class Altitude(Task):
    ...
    def on_run(self, *args, **kwargs):
        if not self.task.finished:
            self.task()
            if shm.desires.depth.get() < self.min_depth:
                self.loge('Min depth reached, preventing surface and exiting')
                shm.desires.depth.set(self.min_depth)
                self.finish()
        else:
            self.finish()
    ...
    def on_first_run(self, vision, *args, **kwargs):
        ...
        if world.stacks_on_table.get() > 0:
            ...
            self.task = SequentialSuccess(
                ...
                Log('Searching for colored table regions'),
                MasterConcurrent(IdentifyObjects(lambda: vision.regions), Search()),
...
class VisionTask(Task):
    ...
    def on_run(self, *args, **kwargs):
        if not self.task.finished:
            try:
                camera = get_camera(shm.recovery_vision.get())
                self.vision(camera)
                self.task()
            except RuntimeError:
...
vision = lambda: Vision(get_camera(shm.recovery_vision.get()))
