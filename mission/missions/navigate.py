...
import aslam
import shm
from constants.config import navigate as constants
...
class Navigate(Task):
    ...
    def on_first_run(self, vision, *args, **kwargs):
        ...
        pipe_heading = pipe_results.heading
        pipe_depth = pipe_results.depth

        self.task = Sequential(GoToPipe(), Conditional(
            Sequential(
                ConcurrentSuccess(CheckDistance(constants.max_distance), Sequential(
                    Retry(lambda: SequentialSuccess(
                        Log('Returning to pipe heading and depth'),
                        Concurrent(
                            Heading(pipe_heading),
                            Depth(pipe_depth, deadband=0.1),
                            finite=False
                        ),
                        ...
                        MasterConcurrent(IdentifyGate(vision), HeadingSearch(pipe_heading)),
                        Zero(),
...
class VisionTask(Task):
    ...
    def on_run(self, *args, **kwargs):
        if not self.task.finished:
            try:
                camera = get_camera(shm.navigate_results.get())
                self.vision(camera)
                self.task()
            except RuntimeError:
