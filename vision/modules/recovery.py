...
opts = [
    ...
    IntOption('table c', constants.table_c, -255, 255),
    IntOption('table blur size', 9, 0, 100),
    IntOption('table morph size', 20, 0, 100),
    IntOption('table min area', constants.table_min_area, 0, 200000),
    IntOption('max clipping distance', 5, 0, 50),

    IntOption('red stack c', constants.red_stack_c, -20, 20),
    IntOption('green stack c', constants.green_stack_c, -40, 40),

    IntOption('red mark c', constants.red_mark_c, -40, 40),
    IntOption('green mark c', constants.green_mark_c, -40, 40),
    IntOption('mark block size', 50, 1, 2000),
...
class Recovery(ModuleBase):
    ...
    def process(self, mat):
        ...
        self.post('bgr_b', self.bgr_b)
        self.post('bgr_g', self.bgr_g)
        self.post('bgr_r', self.bgr_r)

        table_mask = self.table(mat)
        mark_mask = self.marks(mat, table_mask)
        # tower_mask = cv2.bitwise_not(cv2.bitwise_or(table_mask, mark_mask))
        tower_mask = np.ones(mat.shape[:2], np.uint8) * 255
    ...
    def marks(self, mat, table_mask):
        ...
        for color in ['red', 'green']:
            ...
            threshed = cv2.adaptiveThreshold(
                ...
                self.options['{} mark c'.format(color)],
            )
            threshed = cv2.bitwise_and(threshed, threshed, mask=table_mask)
    ...
    def tower(self, mat, mask):
        ...
        for color in ['red', 'green']:
            ...
            threshed = cv2.adaptiveThreshold(
                ...
                self.options['{} stack c'.format(color)],
            )
